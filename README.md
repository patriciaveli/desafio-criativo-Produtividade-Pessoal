# ✨ Desafio Criativo DIO - Acelerando Sua Produtividade Pessoal:

**Produtividade pessoal, foco e organização com apoio de IA.**

## 📌 Objetivo:
Criar um prompt claro e aplicável para orientar uma IA a ajudar pessoas com rotina intensa a organizar prioridades, reduzir sobrecarga e transformar tarefas acumuladas em ações simples e realistas.

## 💡 Passo 1: Intenção:
Quero que gere um plano de produtividade pessoal para pessoas que trabalham, estudam e têm pouco tempo disponível.

## 🧩 Passo 2: Contexto e restrições:
Considere o seguinte contexto: a pessoa pode ter trabalho, estudos, tarefas domésticas, deslocamento, cansaço e compromissos fixos. Muitas vezes, ela não conseguirá fazer tudo no mesmo dia.  
O conteúdo deve ter formato de checklist com 5 a 7 itens, indicando prioridade, tempo estimado, energia necessária e primeira ação simples.  
A IA deve fazer apenas uma pergunta por vez e aguardar a resposta antes de continuar.  
Evite linguagem técnica, listas longas, metas irreais e tom de cobrança.

_

>## 🤖 Passo 3:
Entrega final que reune intenção, contexto, formato e restrições.
> **[!Prompt final]**

```text
Quero que gere um plano de produtividade pessoal para pessoas que trabalham, estudam e têm pouco tempo disponível, com o objetivo de aumentar o foco, reduzir a sobrecarga e criar um plano realista de ação.
Considere o seguinte contexto: a pessoa pode ter trabalho, estudos, tarefas domésticas, deslocamento, cansaço, compromissos fixos e dificuldade para manter constância.
Muitas vezes, ela não conseguirá fazer tudo no mesmo dia.
Antes de montar o plano, faça até cinco perguntas para entender quais tarefas precisam ser feitas, quais são os compromissos fixos, quanto tempo real existe disponível, qual é o nível de energia da pessoa e quais prazos ou consequências tornam algumas tarefas mais urgentes.
Faça apenas uma pergunta por vez e aguarde a minha resposta antes de fazer a próxima.
Não envie todas as perguntas juntas. Depois que tiver informações suficientes, monte o plano.
Depois das respostas, monte um checklist com no máximo 5 a 7 itens, separando as tarefas em três grupos: fazer agora, fazer ainda hoje e deixar preparado para depois.
Para cada tarefa, indique prioridade, tempo estimado, nível de energia necessário e uma primeira ação simples para começar.
Inclua pausas curtas e realistas ao longo do plano, especialmente depois de tarefas que exigem mais concentração.
Se houver tarefas demais para o tempo disponível, ajude a pessoa a escolher o que adiar, simplificar ou eliminar.
Ao final, apresente uma breve avaliação com um ponto forte da organização, um risco de sobrecarga e uma sugestão prática para manter a rotina mais sustentável.
Use linguagem simples, clara, acolhedora e motivadora.
Evite termos técnicos, excesso de informações, listas muito longas, metas irreais, tom de cobrança e recomendações difíceis de aplicar.
```
-

## 🔧 Testando o prompt:
Ao testar o prompt, a IA fez uma pergunta por vez para entender tarefas, compromissos, energia, urgências e tempo disponível. O contexto considerado foi:
* Tarefas: estudar para recolocação em tecnologia, revisar currículo/LinkedIn, responder mensagens importantes, cuidar da casa e incluir exercícios.
* Compromissos: trabalho de segunda a sexta (8h–18h), estudos à noite de segunda a quinta, transporte público de 1h a 1h30 por trajeto, chegada em casa por volta das 23h30.
* Rotina: apertada, nem sempre dá para fazer tudo; fim de semana dedicado à casa e estudos.
* Prioridades: manter estudos, não acumular tarefas importantes, incluir exercícios sem sobrecarga.
* Tempo disponível: limitado e cansativo durante a semana, com algum espaço livre aos sábados e domingos.

-

>## ✍️ Resultado Final do Prompt:
>**A IA criou um plano enxuto, dividido em três grupos:**

## Plano de produtividade pessoal

### ✅ Fazer agora

| Tarefa                                                            | Prioridade |     Tempo | Energia | Primeira ação                                                                      |
| ----------------------------------------------------------------- | ---------- | --------: | ------- | ---------------------------------------------------------------------------------- |
| ☐ Definir a prioridade da semana (estudo, currículo ou mensagens) | Alta       |    10 min | Baixa   | Escreva em um papel ou aplicativo qual será o foco da semana.                      |
| ☐ Responder as mensagens realmente importantes                    | Alta       | 15–20 min | Baixa   | Abra apenas as conversas prioritárias e responda primeiro as que dependem de você. |

**Pausa:** 5 minutos para levantar, beber água ou caminhar um pouco.

### 📌 Fazer ainda hoje

| Tarefa                              | Prioridade |     Tempo | Energia | Primeira ação                                                  |
| ----------------------------------- | ---------- | --------: | ------- | -------------------------------------------------------------- |
| ☐ Fazer uma sessão curta de estudos | Alta       | 25–40 min | Média   | Escolha apenas um assunto e estude sem tentar "adiantar tudo". |
| ☐ Fazer um exercício leve           | Média      | 15–20 min | Média   | Faça uma caminhada, alongamento ou um treino curto em casa.    |

**Pausa:** 5 a 10 minutos antes de iniciar outra atividade.

### 📅 Deixar preparado para depois

| Tarefa                         | Prioridade |     Tempo | Energia | Primeira ação                                                                 |
| ------------------------------ | ---------- | --------: | ------- | ----------------------------------------------------------------------------- |
| ☐ Revisar currículo e LinkedIn | Média      | 30–60 min | Média   | Anote três melhorias que deseja fazer e reserve um período do fim de semana.  |
| ☐ Organizar a semana seguinte  | Média      |    15 min | Baixa   | Defina um horário no sábado ou domingo para decidir o foco da próxima semana. |

## Quando faltar tempo

Em vez de tentar fazer tudo:

* **Adie:** revisão de currículo/LinkedIn se os estudos precisarem de mais atenção naquela semana.
* **Simplifique:** faça 20 a 30 minutos de estudo em vez de uma sessão longa.
* **Elimine:** tarefas que não são urgentes nem importantes naquele momento.

Lembre-se de que manter uma rotina pequena e constante costuma trazer mais resultado do que alternar entre excesso de atividades e períodos sem conseguir fazer nada.

### Avaliação

**Ponto forte:** você já identificou claramente suas prioridades e reconhece os limites da sua rotina, o que ajuda a criar um plano sustentável.

**Risco de sobrecarga:** tentar aproveitar todos os momentos livres para produzir pode aumentar o cansaço e dificultar a constância, principalmente por causa do trabalho, estudos e deslocamentos.

**Sugestão prática:** trate o fim de semana como seu principal período para tarefas que exigem concentração (como estudar e revisar currículo) e use os dias úteis apenas para pequenas ações de manutenção, como responder mensagens, revisar um conteúdo por alguns minutos ou fazer um exercício curto. Dessa forma, a rotina tende a ser mais sustentável e aumenta a chance de você manter o ritmo ao longo das semanas.

---


> ## 🚀 Conclusão:
>Este prompt não trata produtividade como uma lista infinita de obrigações, ele orienta a IA a entender o momento da pessoa, considerar tempo e energia disponíveis, e transformar a rotina em pequenas ações possíveis. Assim, a IA deixa de ser apenas uma ferramenta de resposta e passa a funcionar como uma parceira de organização, ajudando a pessoa a começar com mais clareza, foco e leveza.


